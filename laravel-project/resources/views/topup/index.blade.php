...
@section('title', 'Top-up Saldo - Ad Dashboard')
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-5 mb-5">
    ...
    <div class="card-hover fade-up-3 rounded-2xl p-6 relative overflow-hidden" style="background:#fff; border:1px solid #e2e8f0; box-shadow:0 2px 12px rgba(0,0,0,0.06);">
        <div class="absolute top-0 left-0 right-0 h-0.5 rounded-t-2xl" style="background:linear-gradient(135deg,#6366f1,#8b5cf6);"></div>
        <div class="flex items-center justify-between h-full">
            <div>
                <p class="text-xs font-semibold uppercase tracking-widest text-slate-400 mb-3">Quick Action</p>
                <a href="{{ route('topup.create') }}" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold text-white hover:opacity-90 transition" style="background:linear-gradient(135deg,#6366f1,#8b5cf6); box-shadow:0 4px 12px rgba(99,102,241,0.3);">
...
<div class="fade-up rounded-2xl p-5 mb-5" style="background:#fff; border:1px solid #e2e8f0; box-shadow:0 2px 12px rgba(0,0,0,0.06);">
    <form action="{{ route('topup.index') }}" method="GET" class="flex flex-wrap gap-4 items-end">
        <div class="flex-1 min-w-48">
            ...
            <select name="ad_account" onchange="this.form.submit()" class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-400">
...
<div class="fade-up rounded-2xl overflow-hidden" style="background:#fff; border:1px solid #e2e8f0; box-shadow:0 2px 12px rgba(0,0,0,0.06);">
                ...
                <tr class="table-row-hover border-b border-slate-50">
                    ...
                    <td class="px-6 py-4">
                        <span class="capitalize text-sm font-semibold text-slate-700 inline-flex items-center gap-2">
                            ...
                            @elseif($topup->adAccount->platform=='meta') <i class="fa-brands fa-facebook text-indigo-500"></i>
    ...
    <div class="px-6 py-4 border-t border-slate-100">{{ $topups->links() }}</div>
</div>
@endsection
